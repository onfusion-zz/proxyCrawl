...
function getModulePaths() {
	...
	glob.sync(assetsFolder + "/js/pages/**/*.js", { nonull: false }).forEach((file) => {
		if (file.indexOf("webpack-entry-module.js") !== -1) {
			return;
		}
...
const webpackEntryModules = gulp.series(js, () => {
	return gulp
		.src("./gulpfile.js") // Unique file to run once
		.pipe(once())
		.pipe(
			gulpFn(() =>
				fs.writeFileSync(
					assetsFolder + "/js/webpack-entry-module.js",
					getWebpackEntryModuleBootstrapCode(getModulePaths())
				)
			)
		);
});
...
const devJsWebpack = gulp.series(webpackEntryModules, () => {
	return gulp
		.src(`${assetsFolder}/js/webpack-entry-*.js`)
		.pipe(once())
